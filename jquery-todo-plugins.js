!function(){
    function _MD() {
        ...
        * @param {Object} data: { title, body, replaceBody: boolean } : replaceBody是否自定义body
        ...
        */ 
        $.tips = function ( type, data, cb ){
            ...
            if( typeof type !== 'string' ){
                ...
                throw new Error( '你他么给点正规值能死吗，看不懂注释吗' );
            }
            ...
            var _html = [
                            ...
                            ,'</textarea>'                              //3
                            ,'</div>'                           //4
                            ,'</h2><div readonly onfocus="this.blur()" class="jq-tips-body">' //5
                            ,'</div>'         //6
                        ];
            ...
            if( data.replaceBody )
               html = _html[ 0 ] + className + _html[ 1 ] +  ( data.title || '' ) + _html[ 5 ] + ( data.body || '' ) + _html[ 6 ] + ( types[ 1 ] ? html_ : '' ) + _html[ 4 ];
            else
